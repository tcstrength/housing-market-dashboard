...
def crawl_one_pending(pending_post: PendingPostEntity, crawl_method: CrawlMethod):
    engine = create_engine(app_config.POSTGRES_CONN)
    ...
    crawler = DetailPageCrawler(crawl_method=crawl_method)
...
def crawl_async(engine, num_posts, num_crawlers, crawl_method):
    pending_posts = get_pending_posts(num_posts)
    inputs = []
    for pending_post in pending_posts:
        inputs.append((pending_post, crawl_method))
...
def main(num_posts, num_crawlers, crawl_method):
    engine = create_engine(app_config.POSTGRES_CONN)
    BaseEntity.metadata.create_all(
        engine, tables=[
            PostDetailEntity.__table__,
    ...
    return crawl_async(engine, num_posts=num_posts, num_crawlers=num_crawlers, crawl_method=crawl_method)

if __name__ == "__main__":
    results = main(num_posts=10, num_crawlers=4, crawl_method=CrawlMethod.BASIC)
    print(results)
