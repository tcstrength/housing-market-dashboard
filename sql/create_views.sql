DROP VIEW IF EXISTS rp_post_detail;
CREATE VIEW rp_post_detail AS
WITH get_array AS (
...
SELECT p.*, post_url, post_type, post_title,
       street, ward, district,
       CASE WHEN province LIKE '%Hồ Chí Minh%' THEN
           'Hồ Chí Minh' ELSE province
       END AS province
FROM split_address a
JOIN param_pivoted p ON a.post_id = p.post_id::int